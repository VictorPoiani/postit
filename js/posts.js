...
function startEditPost(card, post) {
    ...
    editBox.innerHTML = `
        <textarea class="composer-input post-edit-textarea" maxlength="2500">${post.content || ''}</textarea>
        <div class="post-edit-actions">
            <span class="post-edit-count">${(post.content || '').length}/2500</span>
    ...
    textarea.addEventListener('input', () => {
        countEl.textContent = `${textarea.value.length}/2500`;
    });
...
function updateCharCount() {
    ...
    const len = input.value.length;
    if (counter) counter.textContent = `${len}/2500`;
